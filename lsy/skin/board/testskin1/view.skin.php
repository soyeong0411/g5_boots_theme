...
;
    ...
    <section id="bo_v_atc">
        ...
        <?php if ($view['wr_1'] && $view['wr_2'] && $view['wr_3']): ?>
            <style type="text/css">
                .quiz p.q_t1 {font-size: 2em;color: black;}
                .quiz p.q_t2 {font-size: .9em;color: red;}
            </style>
            <div class="quiz" id="quiz">
                <p class="q_t1">Q. <?=$view['wr_1']?></p>
                <p class="q_t2">정답포인트 <?=$view['wr_3']?></p>
                <?php if ($view['wr_4'] == 'C'): ?>
                    <span>마감</span>
                    <input type="text" name="q_ans" id="q_ans" value="<?=$view['wr_2']?>" readonly>
                <?php else: ?>
                    <input type="text" name="q_ans" id="q_ans">
                    <input type="button" id="q_btn" value="정답확인" data-wrid="<?=$view['wr_id']?>" data-botable="<?=$bo_table?>">
                <?php endif ?>
            </div>
            <script type="text/javascript">
                var qBtn = document.getElementById('q_btn');
                qBtn.addEventListener('click', function(e) {
                    var qAjaxUrl = '<?=$board_skin_url?>/ajax.quiz.php';
                    var mbId = '<?=$member['mb_id']?>';
                    var qAns = document.getElementById('q_ans').value;
                    var wrId = this.dataset.wrid;
                    var boTable = this.dataset.botable;
